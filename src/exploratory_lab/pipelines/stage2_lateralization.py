"""
# Exploratory Procedure: Stage 2 - Lateralization Coordinate Transformations (Task 30)
# Exploratory Goal: Evaluate if lateralization forms an independent latent axis and test coordinate invariance.
# Input Data: Feature-level data (7 production features). No trial-level modification.
# Parameters: Block A (Mean/Diff rotation), Block B (Asymmetry subspace only).
# Output: `Task_30_Stage2_Report.md` containing performance metrics and loadings.
...
OUTPUT_REPORT = PROJECT_ROOT / "data" / "exploratory" / "reports" / "Task_30_Stage2_Report.md"
...
FEATURES_BLOCK_A = [
    "mean_v4", "diff_v4", "mean_v5", "diff_v5", "asym_dv1_abs", "asym_dv1_rel", "psi_tau"
]

FEATURES_BLOCK_B = [
    "diff_v4", "diff_v5", "asym_dv1_abs", "asym_dv1_rel"
]
...
# ============================================================================
# DATA PREPARATION (Reusing Stage 1 exactly)
# ============================================================================
...
def transform_mean_diff(df):
    """Block A: Rotate Left/Right into Mean/Diff components."""
    tf = df.copy()
    tf['mean_v4'] = (tf['delta_v4_left_residual'] + tf['delta_v4_right_residual']) / 2
    tf['diff_v4'] = tf['delta_v4_left_residual'] - tf['delta_v4_right_residual']
    tf['mean_v5'] = (tf['delta_v5_left_residual'] + tf['delta_v5_right_residual']) / 2
    tf['diff_v5'] = tf['delta_v5_left_residual'] - tf['delta_v5_right_residual']
    return tf

def measure_diff_variance(df_tf, features):
    """Measures the proportion of raw variance explained by diff components."""
    scaler = StandardScaler()
    scaled = scaler.fit_transform(df_tf[features])
    var_total = scaled.var(axis=0).sum()
    
    var_diff_v4 = scaled[:, features.index('diff_v4')].var()
    var_diff_v5 = scaled[:, features.index('diff_v5')].var()
    
    return {
        'diff_v4_pct': (var_diff_v4 / var_total) * 100,
        'diff_v5_pct': (var_diff_v5 / var_total) * 100
    }
...
def main():
    print("=" * 60)
    print("Task 30. Stage 2 - Lateralization Coordinate Transformations")
    print("=" * 60)
    ...
    # 1. Block A (Mean/Diff mapping)
    print("Evaluating Block A (Mean/Diff)...")
    sess_A = transform_mean_diff(df_sess)
    subj_A = transform_mean_diff(df_subj)
    res_A = evaluate_transformation("Block A (Mean/Diff)", sess_A, subj_A, FEATURES_BLOCK_A)
    results.append(res_A)
    
    # Also calculate variance contribution of diffs in Block A
    var_cont = measure_diff_variance(subj_A, FEATURES_BLOCK_A)
    
    # 2. Block B (Asymmetry Subspace)
    print("Evaluating Block B (Asymmetry Only)...")
    res_B = evaluate_transformation("Block B (Asymmetry)", sess_A, subj_A, FEATURES_BLOCK_B)
    results.append(res_B)
    
    # Generage Report
    print("Generating report...")
    report = []
    report.append("# Task 30. Stage 2 — Координатные преобразования латерализации\n")
    ...
    report.append("### Baseline (Left/Right basis)")
    loadings_base = res_base['Loadings']
    report.append("| Feature | PC1 | PC2 |")
    report.append("|---|---|---|")
    for feat, row in loadings_base.iterrows():
    ...
    loadings_b = res_B['Loadings']
    report.append("| Feature | PC1 | PC2 |")
    report.append("|---|---|---|")
    for feat, row in loadings_b.iterrows():
         report.append(f"| `{feat}` | {row['PC1']:.3f} | {row['PC2']:.3f} |")
         
    report.append("\n## 3. Дисперсия Diff-компонент (Блок А)\n")
    report.append(f"- `diff_v4` составляет **{var_cont['diff_v4_pct']:.2f}%** от общей дисперсии нового базиса.")
    report.append(f"- `diff_v5` составляет **{var_cont['diff_v5_pct']:.2f}%** от общей дисперсии нового базиса.")

    report.append("\n## 4. Сценарный анализ латерализации\n")
    report.append("Сценарии:\n")
    
    if res_base['Dim'] == res_A['Dim'] and abs(res_base['PC1%'] - res_A['PC1%']) < 5.0 and (res_B['Dim'] >= 1):
         report.append("1. **Латерализация формирует вторую ось / независима**. (При смене координат метрики стабильны, а изолированное подпространство асимметрии образует устойчивую геометрию с Dim >= 1).")
    elif res_B['Dim'] == 0 or res_B['PC1%'] < 20.0 or res_B['Boot SD'] > 5.0:
         report.append("3. **Асимметрия не образует устойчивой геометрии**. (Изолированное подпространство шумит, низкая дисперсия, Dim = 0 или высокий Boot SD).")
    else:
         report.append("2. **Латерализация вторична по отношению к общей скорости**. (Пересечение сценариев, координатная смена может частично разрушать PC1).")
