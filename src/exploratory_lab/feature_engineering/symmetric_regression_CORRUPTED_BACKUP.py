...
class SymmetricRegressionAnalyzer:
    ...
    def _validate_distributions(self, features_df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        Check skewness, kurtosis, and outliers for all key features.
        ...
        -------
        dict
            Distribution statistics and outlier counts per feature
        \"""
        ...
        for feature in features_to_check:
            ...
            distribution_results[feature] = {
                ...
                'outlier_risk': outlier_pct > 0.05  # > 5% threshold
            }
    ...
    def _document_standardization(self, features_df: pd.DataFrame, 
        ...
        -------
        dict
            Standardization method and statistics per predictor
        \"""
        standardization_doc = {
        ...
        # Document statistics for each predictor pair
        for field in ['left', 'right']:
            pred_median = f'median_dv1_{field}'
            pred_mad = f'mad_dv1_{field}'
    ...
    def _compute_robust_se_all(self, features_df: pd.DataFrame, 
        ...
        features_df : pd.DataFrame
            Feature matrix
        results : dict
            Full analysis results including heteroscedasticity tests
        
        Returns
        -------
        dict
            Robust SE results for each model with heteroscedasticity
        \"""
        robust_results = {}
        ...
        for outcome, hetero_dict in hetero_tests.items():
            # Only compute robust SE if heteroscedasticity detected
            if not hetero_dict.get('heteroscedastic', False):
                continue
            
            # Get field and predictor
    ...
    def _compute_robust_se_hc3(self, X: np.ndarray, y: np.ndarray, 
        ...
        HC3 formula: V_HC3 = (X'X)^(-1) X' Ω X (X'X)^(-1)
        where Ω_ii = e_i² / (1 - h_ii)²
        
        Parameters
        ----------
        X : np.ndarray
            Predictor matrix (n x 1)
        y : np.ndarray
            Outcome vector
        ...
        -------
        dict
            Classical and robust SE, t-stats, p-values
        \"""
        n = len(X)
        ...
        # Robust SE (for slope coefficient, index 1)
    ...
    def generate_report(self) -> str:
        """
        Generate comprehensive numerical report (Task 27.2A Section VI).
        
        Returns
        -------
        str
            Formatted text report with all regression results
        """
        lines = []
        
        lines.append("=" * 80)
        lines.append("TASK 27.2A - SYMMETRIC REGRESSION & RESIDUAL ANALYSIS REPORT")
        lines.append("=" * 80)
        lines.append("")
        
        # I. Linear Regression Results
        lines.append("I. LINEAR REGRESSION (Field-by-Field)")
        lines.append("-" * 80)
        for outcome, stats_dict in self.linear_results.items():
            lines.append(f"\n{outcome.upper()}:")
            lines.append(f"  N: {stats_dict['n']}")
            lines.append(f"  R²: {stats_dict['r_squared']:.4f}")
            lines.append(f"  Adjusted R²: {stats_dict['adj_r_squared']:.4f}")
            lines.append(f"  β: {stats_dict['beta']:.4f} (SE={stats_dict.get('se_beta', np.nan):.4f})")
            lines.append(f"  p-value: {stats_dict.get('p_value', np.nan):.4f}")
            lines.append(f"  Residual variance ratio: {stats_dict['residual_ratio']:.4f}")
        
        lines.append("\n")
        
        # II. Multiple Regression Results
        if self.multiple_results:
            lines.append("II. MULTIPLE REGRESSION (With MAD Control)")
            lines.append("-" * 80)
            for outcome, stats_dict in self.multiple_results.items():
                lines.append(f"\n{outcome.upper()}:")
                lines.append(f"  N: {stats_dict['n']}")
                lines.append(f"  R² (full model): {stats_dict['r_squared']:.4f}")
                lines.append(f"  R² (simple model): {stats_dict['r_squared_simple']:.4f}")
                lines.append(f"  ΔR² (MAD contribution): {stats_dict['delta_r_squared']:.4f}")
                lines.append(f"  β (Median, std): {stats_dict['standardized_beta_median']:.4f}")
                lines.append(f"  β (MAD, std): {stats_dict['standardized_beta_mad']:.4f}")
        
        lines.append("\n")
        
        # III. Residual Correlations
        if self.residual_correlations:
            lines.append("III. RESIDUAL STRUCTURE ANALYSIS")
            lines.append("-" * 80)
            for outcome, corr_dict in self.residual_correlations.items():
                lines.append(f"\n{outcome.upper()} Residuals:")
                for latent_feat, stats_dict in corr_dict.items():
                    independent = "✓ INDEPENDENT" if stats_dict.get('independent', False) else "  -"
                    lines.append(f"  {latent_feat}: r={stats_dict['r']:.4f}, p={stats_dict.get('p_value', np.nan):.4f} {independent}")
        
        lines.append("\n")
        
        # IV. Nonlinearity Tests
        if self.nonlinearity_tests:
            lines.append("IV. NONLINEARITY ASSESSMENT (AIC/BIC)")
            lines.append("-" * 80)
            for outcome, stats_dict in self.nonlinearity_tests.items():
                improves = "✓ NONLINEAR" if stats_dict.get('quad_improves', False) else "  Linear sufficient"
                lines.append(f"\n{outcome.upper()}:")
                lines.append(f"  ΔAIC: {stats_dict['delta_aic']:.2f} {improves}")
                lines.append(f"  ΔBIC: {stats_dict['delta_bic']:.2f}")
        
        lines.append("\n")
        
        # V. Hemispheric Comparisons
        if self.hemispheric_comparisons:
            lines.append("V. HEMISPHERIC ASYMMETRY")
            lines.append("-" * 80)
            for pathway, comp_dict in self.hemispheric_comparisons.items():
                asymmetric = "✓ ASYMMETRIC" if comp_dict.get('r2_asymmetric', False) else "  Symmetric"
                lines.append(f"\n{pathway.upper()}:")
                lines.append(f"  R² (left): {comp_dict['r2_left']:.4f}")
                lines.append(f"  R² (right): {comp_dict['r2_right']:.4f}")
                lines.append(f"  ΔR²: {comp_dict['r2_diff']:.4f} {asymmetric}")
        
        lines.append("")
        lines.append("=" * 80)
        lines.append("END OF TASK 27.2A REGRESSION REPORT")
        lines.append("=" * 80)
        
        # Task 27.2B: Methodological Validation
        if self.heteroscedasticity_tests or self.psi_tau_stability or self.nested_model_tests:
            lines.append("")
            lines.append("")
            lines.append("=" * 80)
            lines.append("TASK 27.2B - METHODOLOGICAL VALIDATION REPORT")
            lines.append("=" * 80)
            lines.append("")
            
            # VI. Heteroscedasticity Tests
            if self.heteroscedasticity_tests:
                lines.append("VI. HETEROSCEDASTICITY ASSESSMENT (Breusch-Pagan Test)")
                lines.append("-" * 80)
                for outcome, test_dict in self.heteroscedasticity_tests.items():
                    hetero_flag = "HETEROSCEDASTIC" if test_dict.get('heteroscedastic', False) else "Homoscedastic"
                    lines.append(f"\n{outcome.upper()}:")
                    lines.append(f"  N: {test_dict['n']}")
                    lines.append(f"  LM statistic: {test_dict['lm_statistic']:.4f}")
                    lines.append(f"  p-value: {test_dict['p_value']:.4f}")
                    lines.append(f"  |residual| vs predictor: r={test_dict['abs_resid_correlation']:.4f}")
                    lines.append(f"  → {hetero_flag}")
                
                lines.append("")
            
            # VII. PSI Tau Stability
            if self.psi_tau_stability:
                lines.append("\nVII. PSI TAU STABILITY ANALYSIS")
                lines.append("-" * 80)
                
                distrib = self.psi_tau_stability.get('distribution', {})
                if distrib:
                    lines.append("\nDistribution:")
                    lines.append(f"  Mean: {distrib.get('mean', np.nan):.4f}")
                    lines.append(f"  SD: {distrib.get('std', np.nan):.4f}")
                    lines.append(f"  Median: {distrib.get('median', np.nan):.4f}")
                    lines.append(f"  Range: [{distrib.get('min', np.nan):.4f}, {distrib.get('max', np.nan):.4f}]")
                    lines.append(f"  CV: {distrib.get('cv', np.nan):.4f}")
                
                psi_obs = self.psi_tau_stability.get('psi_observations', {})
                if psi_obs:
                    lines.append("\nPSI Observations per Subject:")
                    lines.append(f"  Mean count: {psi_obs.get('mean_count', np.nan):.1f}")
                    lines.append(f"  Range: [{psi_obs.get('min_count', np.nan):.0f}, {psi_obs.get('max_count', np.nan):.0f}]")
                    lines.append(f"  Subjects with < 4 observations: {psi_obs.get('subjects_with_insufficient', 0)}")
                
                tau_deps = self.psi_tau_stability.get('tau_dependencies', {})
                if tau_deps:
                    dependent_flag = "DEPENDENT on observation count" if tau_deps.get('dependent_on_count', False) else "Independent"
                    lines.append("\nTau Dependencies:")
                    lines.append(f"  tau vs observation count: r={tau_deps.get('r_vs_observation_count', np.nan):.4f}, p={tau_deps.get('p_vs_observation_count', np.nan):.4f}")
                    lines.append(f"  tau vs PSI range: r={tau_deps.get('r_vs_psi_range', np.nan):.4f}, p={tau_deps.get('p_vs_psi_range', np.nan):.4f}")
                    lines.append(f"  → {dependent_flag}")
                
                lines.append("")
            
            # VIII. Nested Model F-Tests
            if self.nested_model_tests:
                lines.append("\nVIII. NESTED MODEL F-TESTS (MAD Contribution)")
                lines.append("-" * 80)
                for outcome, ftest_dict in self.nested_model_tests.items():
                    mad_sig = "SIGNIFICANT" if ftest_dict.get('mad_significant', False) else "Not significant"
                    mad_size = "MINIMAL (\u0394R\u00b2<0.02)" if ftest_dict.get('mad_minimal', False) else ""
                    lines.append(f"\n{outcome.upper()}:")
                    lines.append(f"  N: {ftest_dict['n']}")
                    lines.append(f"  \u0394R²: {ftest_dict['delta_r_squared']:.4f}")
                    lines.append(f"  F-statistic: {ftest_dict['f_statistic']:.4f}")
                    lines.append(f"  p-value: {ftest_dict['p_value']:.4f}")
                    lines.append(f"  → {mad_sig} {mad_size}")
                
                lines.append("")
            
            lines.append("=" * 80)
            lines.append("END OF TASK 27.2B METHODOLOGICAL VALIDATION")
            lines.append("=" * 80)
