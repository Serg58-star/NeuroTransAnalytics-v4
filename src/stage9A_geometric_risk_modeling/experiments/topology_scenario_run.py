"""
stage9A_geometric_risk_modeling.experiments.topology_scenario_run

Executes the model comparison framework strictly against a TOPOLOGY-dependent 
synthetic dataset to verify the architecture correctly assigns the TOPOLOGY_DEPENDENT verdict.
"""
...
from stage9A_geometric_risk_modeling.common.data_loader import generate_topology_dependent_data
...
def main():
    ...
    n_samples = 10000 # overridden from config to allow KDE sufficient bandwidth density
    seed = config['dataset']['base_seed']
    
    print("Generating Synthetic Benchmark (Topology-Dependent Data)...")
    df = generate_topology_dependent_data(n_samples=n_samples, seed=seed)
    
    dataset_info = {
        'n_samples': n_samples,
        'condition_type': 'Binary (Topology Dependent pockets)',
    ...
    report_path = os.path.join(report_dir, 'task39_1_topology_scenario_report.md')
    
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(report_md)
        
    print(f"Task 39.1 Topological Scenario Execution Complete. Output: {report_path}")
