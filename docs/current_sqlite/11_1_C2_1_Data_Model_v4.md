...
В физической реализации v4 эта сущность **реализована как таблица `users`**.
...
В текущей физической модели v4 данные сессий **представлены в таблице `trials`**.
...
Физически эти события **реализованы как записи в таблицах `metadata_*` и `warmup_*`**.
...
В физической реализации v4 реакции **представлены группами полей внутри таблицы `trials`**.
...
Примечание: Физическая реализация v4 в SQLite полностью придерживается этих логических принципов через соответствующую структуру таблиц.
